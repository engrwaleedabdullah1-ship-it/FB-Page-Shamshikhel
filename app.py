...
with st.sidebar:
    ...
    font_style = st.selectbox("Font Style (English/Roman):", ["Regular", "Bold", "Italic"])
...
def get_font_path(is_urdu, style_choice):
    # Always use the Urdu font if the text is in Urdu
    if is_urdu and os.path.exists("urdu_font.ttf"):
        return "urdu_font.ttf"
    
    # Map the dropdown choice to your uploaded files
    style_map = {
        "Regular": "font_regular.ttf",
        "Bold": "font_bold.ttf",
        "Italic": "font_italic.ttf"
    }
    
    target_font = style_map.get(style_choice, "font_regular.ttf")
    
    # Check if the specific file exists, if not, fallback safely
    if os.path.exists(target_font):
        return target_font
    elif os.path.exists("font_regular.ttf"):
        return "font_regular.ttf"
    return None

# --- TEXT WRAPPING & SCALING ENGINE ---
def draw_auto_scaled_text(draw, text, max_width, max_height, start_y, is_urdu, color, max_font, selected_style):
        ...
        with st.spinner("Translating and designing your post..."):
            ...
            try:
                if language == "English":
                    ...
                    st.info("Note: True Roman Urdu translation requires advanced APIs; using standard formatting.")
                    final_text = user_text 
            ...
            try:
                ...
                # Automatically resize logo based on the canvas size
                logo_size = int(canvas_w * 0.15) 
                logo = logo.resize((logo_size, logo_size), Image.LANCZOS)
                
                # Paste in top right corner with professional padding
                padding = 40
                img.paste(logo, (canvas_w - logo_size - padding, padding), logo)
            except FileNotFoundError:
                try:
                    # Fallback just in case you named it .png
                    ...
                    st.warning("⚠️ Logo file not found. Make sure it is named 'logo.jpg' or 'logo.png' in GitHub!")

            # 6. Draw Auto-Scaled Text
            text_box_width = canvas_w - 200
            text_box_height = canvas_h - 300
            start_y = int(canvas_h * 0.25) # Start text a quarter way down the page
            
            draw_auto_scaled_text(draw, final_text, canvas_w, text_box_height, start_y, is_urdu, rgb_text_color, max_font_size, font_style)
            ...
            with col2:
                ...
                st.download_button(
                    ...
                    data=byte_im,
                    file_name="Discover_Shamshikhel_Post.png",
                    mime="image/png",
